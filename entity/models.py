...
class CamembertForEntity(CamembertPreTrainedModel):
    ...
    def _get_span_embeddings(self, input_ids, spans, token_type_ids=None, attention_mask=None):
        ...
        outputs = self.camembert(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids)
        ...
        return spans_embedding  # (batch, num_spans, hidden*2 + width_emb)
    ...
    def forward(self, input_ids, spans, spans_mask, spans_ner_label=None, token_type_ids=None, attention_mask=None):
        ...
        for layer in self.ner_classifier:
            hidden = layer(hidden)
        logits = hidden  # (batch, num_spans, num_labels)
...
class EntityModel():
    ...
    def move_model_to_cuda(self, gpu_id: int = 0):
        if torch.cuda.is_available():
            self._model_device = torch.device(f'cuda:{gpu_id}')
            torch.cuda.set_device(self._model_device)
            logger.info(f'Using device: {self._model_device}, torch.cuda: {torch.version.cuda}, '
                        f'GPU: {torch.cuda.get_device_name(gpu_id)}')
            self.bert_model.to(self._model_device)
            if torch.cuda.device_count() > 1:
                logger.info(f'#GPUs = {torch.cuda.device_count()}, enabling DataParallel')
                self.bert_model = torch.nn.DataParallel(self.bert_model, device_ids=[gpu_id])
    ...
    def _get_input_tensors_batch(self, samples_list, training=True):
        ...
        for tokens_tensor, bert_spans_tensor, spans_ner_label_tensor in zip(
            ...
            if tokens_pad_length > 0:
                ...
                pad = torch.full([1, tokens_pad_length], pad_id, dtype=torch.long)
    ...
    def run_batch(self, samples_list, try_cuda=True, training=True):
        ...
        tokens_tensor, attention_mask_tensor, bert_spans_tensor, spans_mask_tensor, spans_ner_label_tensor, sentence_length = \
        ...
        if training:
            ...
            predicted, pred_prob, hidden = [], [], []
            for i, sample in enumerate(samples_list):
                ner_i, prob_i, lh_i = [], [], []
                for j in range(len(sample['spans'])):
                    ner_i.append(predicted_label[i][j])
                    prob_i.append(ner_logits[i][j].cpu().numpy())
                    lh_i.append(last_hidden[i][j])
                predicted.append(ner_i)
                pred_prob.append(prob_i)
                hidden.append(lh_i)
