...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', type=str, required=True,
                    help="dataset key used to fetch label map, e.g., custom")
    ...
    parser.add_argument('--model', type=str, default='camembert-base', 
    ...
    args.train_data = os.path.join(args.data_dir, 'train_pure.jsonl')
    args.dev_data = os.path.join(args.data_dir, 'valid_pure.jsonl')
    args.test_data = os.path.join(args.data_dir, 'test_pure.jsonl')
    ...
    if args.do_train:
        ...
        best_result = 0.0
                ...
                # 将 CamemBERT 编码器与任务层（FFNN、width_embedding 等）拆开设学习率
        encoder_param_ids = {id(p) for p in model.bert_model.camembert.parameters()}  # 只针对 camembert 主干
        base_params, task_params = [], []
        for n, p in model.bert_model.named_parameters():
            (base_params if id(p) in encoder_param_ids else task_params).append(p)

        logger.info(f"LR groups: base={len(base_params)} params, task={len(task_params)} params")
        optimizer_grouped_parameters = [
            {'params': base_params, 'lr': args.learning_rate},            # encoder: camembert.*
            {'params': task_params, 'lr': args.task_learning_rate},       # head: ner_classifier.*, width_embedding.*
        ]
